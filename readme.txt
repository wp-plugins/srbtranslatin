...
Pay attention that if you change this option, all previous urls containing script selection will become invalid. It is best to set this before site is heavily indeksed or externally linked.
...
bz default, SrbTranslatin uses priority 99 to make good chance to be at the end of executing plugin list. If for some reason it does not work well, user can change priority. 

There is config?example.php in SrbTransLatin direcotry. User should copy that to config.php and edit new file. There is a line saying:
